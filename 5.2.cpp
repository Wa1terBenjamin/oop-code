/*1、定义类A，有数据成员name（char*），用于存放姓名。有show函数，用于显示名字。有构造函数及析构函数，其中析构函数除完成必要功能外，还显示“destructor
A”。
2、由A派生B、C类，B增加数据成员年龄（int），C增加数据成员性别（char）；均有一个print函数，分别只能显示年龄或性别。B、C类是否需要构造函数,请自行决定，如果需要构造函数，函数体必须为空。析构函数必须有，但只有一条语句，显示"destructor
B"或"destructor C"。 3、定义manage类，有：
...
class manager {
   public:
    ...
    void add(int index, const char* n, char s) {
        array[index] = new C(n, s);
        len++;
    }
    void display() {
        for (int i = 0; i < sizeof(array) / sizeof(A*); i++) {
