...
// 提示：
// 1、常量字符串定义语句char * name
// ＝"abc"早期C++编译器可正常编译，但后续版本认为有错。因为常量字符串”abc”的类型是string
// literal（字符串文字），而name是指向char变量的指针变量，所以直接赋给name有类型不一致问题，而C++11版本不进行隐含类型转换，因此会告警。可以改为char
// * name ＝(char*)"abc"，或者const char * name ＝"abc"
...
int main() {
    const char* p = "abc";
    cout << p << endl;
    char* q = new char[strlen(p) + 2]{'d', 'e', 'f', '\0'};
    cout << q << " " << strlen(p) + 2 << endl;
    delete[] q;
    strcpy(q, p);
    char c;
    cin >> c;
